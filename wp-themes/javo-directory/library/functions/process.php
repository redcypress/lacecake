	...
	public function add_user_callback(){
		...
		if( !is_wp_error($user_id) ){
			...
			if( isset( $_POST['post_id'] ) && (int)$_POST['post_id'] > 0 ){
				...
				$parent_post_id = (int)get_post_meta( $post_id, 'parent_post_id', true);
				update_post_meta($post_id			, 'approve', 'approved');
				update_post_meta($parent_post_id	, 'claimed', 'yes');
			}
	...
	**/
	public function add_item(){
		...
		if( (int)$javo_this_post > 0 ){
			...
			update_post_meta($javo_this_post, "latlng"				, $javo_latLng);
			update_post_meta($javo_this_post, "video"				, $javo_video);
			update_post_meta($javo_this_post, 'directory_meta'		, $javo_directory_meta);
